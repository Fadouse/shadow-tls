...
| **帧填充策略** | 随机化边界 [5,13] + 5% 尾部 | 固定边界 8 + 2% 尾部 | 可配置 PaddingScheme | Vision 填充 | QUIC 帧 | 前 8 次 0-255B | HTTP |
| **帧合并 (Coalescing)** | 有 (sleep-before-read) | 无 | 无 | 无 | QUIC 天然 | HTTP/2 天然 | HTTP 天然 |
| **跨连接指纹** | 随机化过渡点 | 固定 8 包可统计 | 每连接独立 | 无特殊处理 | 每连接独立 | 每连接独立 | N/A |
| **多路复用打散** | 有 (多流合并为一条 TLS 记录) | 无 | 有 | 无 | 原生 | 有 | 有 |

> **说明**: TLS-in-TLS 检测的核心是识别加密隧道中嵌套的 TLS 握手模式 (短-长-短 帧序列)。ShadowTLS 通过填充 + 帧合并 + 多路复用三层防护消除此模式; AnyTLS 和 Naiveproxy 承认此问题"从根本上难以完全消除"。
...
| 协议 | 已知检测方法 | 严重程度 | 备注 |
|------|-----------|:--------:|------|
| **ShadowTLS (本项目)** | 握手服务器 alert 时序分析 (理论) | 低 | 尚无公开工具; 多路复用进一步降低检测面 |
...
| **TLS-in-TLS 抗性** | **9** | 5 | 7 | 6 | N/A | 7 | N/A |
| **协议封锁风险** | 低 | 低 | 低 | 低 | **高** | 低 | **高** |
| **部署复杂度** | 低 | 低 | 低 | 中 | 低 | 高 | 中 |
| **加权综合** | **9.1** | 6.6 | 7.5 | 7.7 | 6.5 | 7.8 | 4.0 |
